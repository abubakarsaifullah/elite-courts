...
export async function POST(request: Request) {
  try {
    ...
    if (!sport || !duration || !clientName || !phone) {
      return NextResponse.json({ ok: false, message: "Please check the booking details." }, { status: 400 });
    }

    if (!isDateInBookingWindow(payload.date)) {
      return NextResponse.json({ ok: false, message: "Selected date is outside the allowed booking window." }, { status: 400 });
    }
    ...
    const booking = await createCalendarBooking({
      bookingId: await generateUniqueBookingId(sport.id),
